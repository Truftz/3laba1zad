...
# Секретный ключ от Google reCAPTCHA
...
def swap_image_parts(image: Image.Image, operation: str) -> Image.Image:
    ...
    try:
        ...
        if operation == "swap_lr":
            ...
            mid = width // 2
            # Копируем части чтобы избежать проблем с размерами
            ...
            new_array[:, :mid] = right_part
            new_array[:, mid:mid + left_part.shape[1]] = left_part
            ...
            if upper_part.shape[0] != lower_part.shape[0]:
                ...
                upper_part = upper_part[:min_height, :]
                lower_part = lower_part[:min_height, :]

            # Создаем новый массив
            new_array = np.zeros_like(img_array)
            new_array[:mid, :] = lower_part
            new_array[mid:mid + upper_part.shape[0], :] = upper_part

            return Image.fromarray(new_array)

        return image  # Если операция не распознана
...
def save_image_to_static(image: Image.Image, filename: str) -> str:
    ...
    try:
        ...
        # Создаем файл ошибки если его нет
...
async def process_image(
    ...
    try:
        print(f"Processing image: {file.filename}, operation: {operation}")
